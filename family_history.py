...
def print_death_age(people_dict):
    ...
    for key in people_dict:
        print(f"Name: {people_dict[key][0]}, Age at death: {people_dict[key][3]-people_dict[key][2]}, Birth year: {people_dict[key][2]}, Death year: {people_dict[key][3]}")
    return
...
def count_genders(people_dict):
    ...
    females=0
    for key in people_dict:
        gender=people_dict[key][1]
        if gender == "F": females+=1
        if gender == "M": males+=1
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key in marriages_dict:
        husband=people_dict[marriages_dict[key][0]]
        wife=people_dict[marriages_dict[key][1]]
        print(f"{husband[0]} age {marriages_dict[key][2]-husband[2]} >> {marriages_dict[key][2]} << {wife[0]} age {marriages_dict[key][2]-wife[2]}\n")
    return

def count_marriages(marriage_dict,people_dict):
    marriage_count_dict={}
    for key in marriage_dict:
        marriage_count_dict[marriage_dict[key][0]]=0
        marriage_count_dict[marriage_dict[key][1]]=0
    for key in marriage_dict:
        marriage_count_dict[marriage_dict[key][0]]+=1
        marriage_count_dict[marriage_dict[key][1]]+=1
    sorted_dict=dict(sorted(marriage_count_dict.items(), key=lambda item: item[1], reverse=True))
    for key in sorted_dict:
        print(f"{people_dict[key][0]} was married {sorted_dict[key]} time(s).")
